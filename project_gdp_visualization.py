...
world_bank_data=read_csv_as_nested_dict("isp_gdp.csv","Country Name",",",'"')
...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    list3 = []
    list4 = []
    for out_value in gdp_countries:
        count = 0
        for inner_value in gdp_countries[out_value].values():
            if inner_value == '':
                count += 1
        if count == 57:
            for value in plot_countries.values():
                if value == out_value:
                    for key in plot_countries:
                        if plot_countries[key] == out_value:
                            list1.append(key)  # list.1为在绘图库中和世行中都有国名，但是在世行中没有对应的GDP数据.
            ...
            for value1 in plot_countries.values():
                if value1 == out_value:
                    for key1 in plot_countries:
                        if plot_countries[key1] == out_value:
                            list2.append(key1)
                            list3.append(value1)
                            plotcountriesGDP = dict(zip(list2, list3))  # 在世行中有数据的绘图库国家代码


    counter = -1
    for the_diffrence in plot_countries.values():
        counter += 1#丢失序号
        if the_diffrence not in list3:
            list4.append(list(plot_countries)[counter])
    missing_from_world_bank = set(list4)  # 在世行中无对应国家名的绘图库国家代码及绘图库和世行都有对应的国名但是在世行中无GDP数据的国家代码


    return (plotcountriesGDP, missing_from_world_bank)#返回结果
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
   ...
   """
    name_from_pygal_countries = list(reconcile_countries_by_name(plot_countries, gdpinfo)[0].values())
    code_from_pygal_countries = list(reconcile_countries_by_name(plot_countries, gdpinfo)[0])
    Auxiliary_list = []
    log_GDP = []
    for country_name in name_from_pygal_countries:
        for the_inner_of_dic in gdpinfo.values():
            if the_inner_of_dic['Country Name'] == country_name:
                ...
                Auxiliary_list = []
    Container_about_GDP = dict(zip(code_from_pygal_countries, log_GDP))  # 字典(键为绘图库国家代码和值为世行GDP)
    fuzhu_list1 = []
    fuzhu_list2 = []
    for key in Container_about_GDP:
        ...
        if 1 < count < 57:  # isp_gdp.csv中2016年每一个都没数据，所以count至少为2
            fuzhu_list1.append(key)
            fuzhu_list2.append((Container_about_GDP[key]))


    Container_about_part_of_GDP = dict(zip(fuzhu_list1, fuzhu_list2))  # 年份无数据的绘图库国家代码
    Contries_code_about_part = [list(Container_about_part_of_GDP.items())[m][0] for m in
                                range(len(Container_about_part_of_GDP))
                                if list(Container_about_part_of_GDP.items())[m][1][int(year) - 1960] == '']
    Container_about_GDP_on_part_year = set(Contries_code_about_part)  # 随着year而变化的年份无数据的绘图库国家代码
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    world_map.render_to_file(map_file)#map函数
...
render_world_map(world_bank_data,pygal_countries,year,'isp_gdp_world_name_1978.svg')
